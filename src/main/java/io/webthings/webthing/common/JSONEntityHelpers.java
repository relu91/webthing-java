...
public class JSONEntityHelpers {
    ...
    public static void addSingleItemOrList(String name,
                                           Collection<String> l,
                                           JSONObject o) {
        if (l != null) {
            if (l.size() == 1) {
    ...
    public static void addRequiredString(String name, String val, JSONObject o)
    ...
    public static void addRequiredJSONEntity(String name,
                                             JSONEntity e,
                                             JSONObject o) throws WoTException {
        if (e != null) {
            o.put(name, e.asJSON());
        }
    }
    ...
    public static void addJSONEntityCollection(String name,
                                               List<? extends JSONEntity> c,
                                               JSONObject o) {
        if (c != null && c.size() > 0) {
    ...
    public static void addJSONEntityCollection(String name,
                                               Map<String, ? extends JSONEntity> c,
                                               JSONObject o) {
        if (c != null && c.size() > 0) {
    ...
    public static <T extends JSONEntity, COLL extends Collection<T>> COLL readEntityCollection(
            JSONObject root,
            String name,
            Class<T> cls,
            Class<COLL> ccls) {
        ...
        try {
            ...
            if (o != null) {
                ...
                for (int i = 0; i < o.length(); ++i) {
                    ...
                    final T retType = (T)cls.newInstance().fromJSON(itm);
                    ret.add(retType);
                }
            }
        } catch (Exception e) {
    ...
    public static <T extends JSONEntity, COLL extends Map<String, T>> COLL readEntityMap(
            JSONObject root,
            String name,
            Class<T> cls,
            Class<COLL> ccls) {
            ...
            if (o != null) {
                ...
                for (final String key : o.keySet()) {
                    if (cls.isInstance(SecurityScheme.class) == false) {
                        ...
                        final T retType = (T)cls.newInstance().fromJSON(child);
                        ret.put(key, retType);
                    } else {
                        final JSONObject child = o.getJSONObject(key);
                        final T retType = (T)SecurityScheme.newInstance(o);
                        ret.put(key, retType);
                    }
                }
            }
    ...
    public static <T extends JSONEntity> T readEntity(JSONObject root,
                                                      String name,
                                                      Class<T> cls) {
        T ret = null;
        try {
            ...
            if (o != null) {
                if (cls.getName().equals(DataSchema.class.getName())) {
                    ret = (T)DataSchema.newInstance(o);
                } else {
                    final T c = cls.newInstance();
                    ret = (T)c.fromJSON(o);
                }
            }
        } catch (Exception e) {
    ...
    public static <T> T readObject(JSONObject root, String name, Class<T> cls) {
        T ret = null;
        try {
    ...
    public static <T, COLL extends Collection<T>> COLL readObjectSingleOrList(
            JSONObject root,
            String name,
            Class<T> cls,
            Class<COLL> ccls) {
    ...
    public static <T, COLL> COLL readCollection(JSONObject root,
                                                String name,
                                                Class<T> cls,
                                                Class<COLL> ccls) {
        COLL ret = null;
        try {
            ...
            if (value != null) {
                ...
                if (value instanceof JSONArray) {
                    ...
                    final JSONArray ja = (JSONArray)value;
                    final Collection coll = (Collection<T>)ret;
                    for (final Object itm : ja) {
                        final T t = (T)itm;
                        coll.add(t);
                    }
                ...
                if (value instanceof JSONObject) {
                    ...
                    final Map<String, T> coll = (Map<String, T>)ret;
                    for (final String key : child.keySet()) {
                        final T collValue = (T)child.get(key);
    ...
    public static <T extends JSONEntity, COLL extends Collection<T>> COLL readSingleEntityOrCollection(
            JSONObject root,
            String name,
            Class<T> cls,
            Class<COLL> ccls) {
        final T ent = readEntity(root, name, cls);
        COLL ret = null;
        if (ent == null) {
            //try collection 
    ...
    public static void addJSONSingleEntityOrCollection(String name,
                                                       List<? extends JSONEntity> c,
                                                       JSONObject o) {
        if (c == null) {
            return;
    ...
    public static java.net.URI readURI(JSONObject root, String name)
            throws WoTException {
        try {
